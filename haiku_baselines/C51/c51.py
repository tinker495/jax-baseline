...
class C51(Q_Network_Family):
    ...
    def _loss(self, params, obses, actions, target_distribution, weights, key):
        distribution = jnp.clip(jnp.squeeze(jnp.take_along_axis(self.get_q(params, obses, key), actions, axis=1)),1e-5,1.0)
    ...
    def _target(self,params, target_params, obses, actions, rewards, nxtobses, not_dones, key):
        ...
        #print(jnp.sum(next_distribution*self.categorial_bar,axis=1)[:5])
        #print(rewards[:5])
        #print(not_dones[:5])
        #print(jnp.sum(target_distribution*self.categorial_bar,axis=1)[:5])
