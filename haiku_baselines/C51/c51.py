...
class C51(Q_Network_Family):
	def __init__(self, env, gamma=0.995, learning_rate=3e-4, buffer_size=100000, exploration_fraction=0.3, 
				 ...
				 prioritized_replay_alpha=0.9, prioritized_replay_beta0=0.4, prioritized_replay_eps=1e-3, 
	...
	def _target(self, params, target_params, obses, actions, rewards, nxtobses, not_dones, key):
		...
		if self.double_q:
			next_action_q = jnp.sum(self.get_q(params,nxtobses,key)*self._categorial_bar,axis=2)
		...
		if self.munchausen:
			...
			next_categorial = not_dones * (self.categorial_bar - jnp.sum(pi_next * tau_log_pi_next, axis=1, keepdims=True))
