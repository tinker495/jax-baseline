        ...
        self._storage = dict(
            [
                (
                    key,
                    np.zeros(
                        (self.worker_size, self._maxsize, *self.obsdict[key]["shape"]),
                        dtype=self.obsdict[key]["dtype"],
                    ),
                )
            ...
            + [
                (
                    "rewards",
                    np.zeros((self.worker_size, self._maxsize, 1), dtype=np.float32),
                ),
                (
                    "actions",
                    np.zeros(
                        (self.worker_size, self._maxsize, *self.action_space),
                        dtype=np.float32,
                    ),
                ),
            ]
                ...
                (
                    ...
                    np.zeros(
                        (
                            self.worker_size,
                            self._maxsize,
                            *self.nextobsdict[key]["shape"],
                        ),
                        dtype=self.nextobsdict[key]["dtype"],
                    ),
                )
        ...
        self._storage = dict(
            [
                (
                    key,
                    np.zeros(
                        (self._maxsize, *self.obsdict[key]["shape"]),
                        dtype=self.obsdict[key]["dtype"],
                    ),
                )
                for key in self.obsdict
            ...
            + [
                ...
                (
                    "actions",
                    np.zeros((self._maxsize, *self.action_space), dtype=np.float32),
                ),
            ]
            + [
                (
                    key,
                    np.zeros(
                        (self._maxsize, *self.nextobsdict[key]["shape"]),
                        dtype=self.nextobsdict[key]["dtype"],
                    ),
                )
                for key in self.nextobsdict
...
class PrioritizedReplayBuffer(ReplayBuffer):
    def __init__(
        ...
        super(PrioritizedReplayBuffer, self).__init__(
            size, observation_space, worker_size, action_space
        )
        assert alpha >= 0
        self._alpha = alpha
...
class EpisodicReplayBuffer(ReplayBuffer):
    ...
    def add(self, obs_t, action, reward, nxtobs_t, done, terminal):
        ...
        for nidx, epkey in zip(nxt_idxs, episode_keys):
            ...
            if t:
                ...
                worker = episode_array[0]
                episode = episode_array[1]
                del self.episodes[(worker, episode)]
        self._storage["episodes"][nxt_idxs] = np.concatenate(
            [
                np.array(episode_keys, dtype=np.int32),
                np.expand_dims(np.array(eplens), axis=1),
            ],
            axis=1,
        )
        self._add(nxt_idxs, obs_t, action, reward, nxtobs_t, done, terminal)
    ...
    def _encode_sample(self, idxes: Union[List[int], np.ndarray]):
        ...
        for idx in idxes:
            ...
            worker = episode_array[0]
            episode = episode_array[1]
            episode_index = episode_array[2]
            nstep_idxs = self.episodes[(worker, episode)][
                episode_index : (episode_index + self.n_step)
            ]
            gamma = self.gamma
...
class PrioritizedEpisodicReplayBuffer(EpisodicReplayBuffer):
    def __init__(
        ...
        :param size: (int)  Max number of transitions to store in the buffer. When the buffer overflows the old
                memories are dropped.
        """
        super(PrioritizedEpisodicReplayBuffer, self).__init__(
            size, observation_space, worker_size, action_space, n_step, gamma
        )
        assert alpha >= 0
        self._alpha = alpha
