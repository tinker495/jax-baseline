...
class FractionProposal(hk.Module):
    ...
    def __call__(self,feature):
        ...
        log_probs = hk.Sequential([
                    hk.Linear(self.node) if i%2 == 0 else jax.nn.leaky_relu for i in range(2*self.hidden_n - 1)
                    ]+[jax.nn.log_softmax]
                    )
...
def visual_embedding(mode="simple"):
    if mode == "normal":
        ...
        net_fn = lambda x: hk.Sequential([
                    hk.Conv2D(512, kernel_shape=[3, 3], stride=[1, 1], padding='SAME'), jax.nn.leaky_relu,
                    hk.Conv2D(512, kernel_shape=[3, 3], stride=[1, 1], padding='SAME'), jax.nn.leaky_relu,
                    hk.Flatten()
                    ])(x)
    elif mode == "none":
