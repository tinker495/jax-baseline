...
def visual_embedding(mode="simple"):
    if mode == "normal":
        ...
        net_fn = lambda x: hk.Sequential([
                    hk.Conv2D(16, kernel_shape=[3, 3], stride=[1, 1], padding='VALID'), jax.nn.leaky_relu,
                    hk.Flatten()
                    ])(x)
    elif mode == 'slide':
        net_fn = lambda x: hk.Sequential([
                    hk.Conv2D(512, kernel_shape=[3, 3], stride=[1, 1], padding='SAME'), jax.nn.tanh,
                    hk.Conv2D(512, kernel_shape=[3, 3], stride=[1, 1], padding='VALID'), jax.nn.tanh,
                    hk.Conv2D(512, kernel_shape=[1, 1], stride=[1, 1], padding='VALID'), jax.nn.tanh,
                    hk.Flatten()
                    ])(x)
    elif mode == "none":
        net_fn = hk.Flatten()
