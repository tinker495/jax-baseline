...
def get_gaes(rewards, dones, terminals, values, next_values, gamma, lamda):
  ...
  last_gae_lam = delta
  _, advs = jax.lax.scan(f, jnp.array(last_gae_lam), (rewards[:-1], dones[:-1], values[:-1], next_values[:-1], terminals[:-1]),reverse=True)
  advs = jnp.append(advs, jnp.expand_dims(last_gae_lam,axis=1), axis=0)
  target = advs + values
  return advs, target
