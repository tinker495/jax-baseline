...
class ACER(Actor_Critic_Policy_Gradient_Family):
    ...
    def get_memory_setup(self):
        if not self.prioritized_replay:
            self.replay_buffer = ReplayBuffer(self.buffer_size,self.observation_space, self.worker_size,
                                              1 if self.action_type == 'discrete' else self.action_size,self.n_step, self.gamma)
        else:
            self.replay_buffer = PrioritizedReplayBuffer(self.buffer_size,self.observation_space,self.prioritized_replay_alpha,
                                                         self.worker_size, 1 if self.action_type == 'discrete' else self.action_size, self.n_step, self.gamma)
    ...
    def action_discrete(self,obs,steps):
        ...
        return prob, np.expand_dims(np.stack([np.random.choice(self.action_size[0],p=p) for p in prob],axis=0),axis=1)
    ...
    def learn_unity(self, pbar, callback=None, log_interval=100):
        ...
        for steps in pbar:
            ...
            self.scores += reward
            self.replay_buffer.add(old_obses, actions, np.expand_dims(reward,axis=1), nxtobs, np.expand_dims(done,axis=1), np.expand_dims(terminal,axis=1))
            if term_on:
                if self.summary:
    ...
    def learn_gymMultiworker(self, pbar, callback=None, log_interval=100):
        ...
        for steps in pbar:
            ...
            self.replay_buffer.add([state], actions, np.expand_dims(rewards,axis=1), [nxtstates], np.expand_dims(dones,axis=1), np.expand_dims(terminals,axis=1))
            self.scores += rewards
            state = next_states
