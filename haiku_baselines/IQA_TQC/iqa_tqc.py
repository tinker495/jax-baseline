...
class IQA_TQC(Deteministic_Policy_Gradient_Family):
    ...
    def _get_update_data(self,params,feature,key = None) -> jnp.ndarray:
        ...
        sample_prob = jax.nn.softmax(jnp.mean(jnp.square(jnp.expand_dims(actions,axis=3) - jnp.expand_dims(actions, axis=1)),axis=(2,3))) #[ batch x tau ]
        log_prob = jnp.log(jnp.take_along_axis(sample_prob, jnp.squeeze(sample_choice,axis=2), axis=1)) #[ batch x 1]
        pi = jax.nn.tanh(jnp.squeeze(jnp.take_along_axis(actions, sample_choice, axis=1),axis=1))#[ batch x action]
