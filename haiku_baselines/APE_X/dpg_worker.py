...
class Ape_X_Worker(object):
	...
	def run(self, local_size, buffer_info, network_builder, actor_builder, param_server, logger_server, update, stop , eps = 0.05):
		try:
			...
			get_abs_td_error, actor, get_action, random_action, noise, key_seq = actor_builder()

			get_abs_td_error = jax.jit(partial(get_abs_td_error, actor_model, cricit_model, preproc))
