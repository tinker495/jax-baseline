...
from haiku_baselines.common.cpprb_buffers import MultiPrioritizedReplayBuffer
...
class Ape_X_Deteministic_Policy_Gradient_Family(object):
	...
	def discription(self):
		return "buffer len : {} loss : {:.3f} |".format(len(self.replay_buffer), np.mean(self.lossque))
		
	def learn(self, total_trainstep, callback=None, log_interval=1000, tb_log_name="APE_X_DPG",
	...
	def learn_gym(self, pbar, callback, log_interval):
		...
		self.logger_server.add_multiline.remote([self.exploration_initial_eps**(1 + self.exploration_decay * idx / (worker_num-1)) for idx in range(worker_num)])
		jobs = []
