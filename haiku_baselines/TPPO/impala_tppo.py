...
class IMPALA_TPPO(IMPALA_Family):
	...
	def preprocess(self, params, key, obses, actions, mu_log_prob, rewards, nxtobses, dones, terminals):
		...
		actions = jnp.vstack(actions); vs = jnp.vstack(vs); prob = jnp.vstack(prob); pi_prob = jnp.vstack(pi_prob); rho = jnp.vstack(rho); adv = jnp.vstack(adv)
	...
	def _train_step(self, params, opt_state, key, ent_coef, obses, actions, mu_log_prob, rewards, nxtobses, dones, terminals):
		obses, actions, vs, old_prob, old_act_prob, rho, adv = self.preprocess(params, key, obses, actions, mu_log_prob, rewards, nxtobses, dones, terminals)
		def i_f(idx, vals):
			...
			def f(updates, input):
				...
				use_key, key = jax.random.split(key)
				(total_loss, (critic_loss, actor_loss, entropy_loss)), grad = jax.value_and_grad(self._loss,has_aux = True)(params, obs, act, vs, old_prob, old_act_prob, adv, ent_coef, use_key)
				updates, opt_state = self.optimizer.update(grad, opt_state, params=params)
				params = optax.apply_updates(params, updates)
				return (params, opt_state, key), (critic_loss, actor_loss, entropy_loss)
			updates, losses  = jax.lax.scan(f, (params, opt_state, key), (obses_batch, actions_batch, vs_batch, old_prob_batch, old_act_prob_batch, adv_batch))
			params, opt_state, key = updates
			cl, al, el = losses
	...
	def learn(self, total_trainstep, callback=None, log_interval=10, tb_log_name="IMPALA_TPPO",
