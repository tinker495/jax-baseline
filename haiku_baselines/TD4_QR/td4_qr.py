...
class TD4_QR(Deteministic_Policy_Gradient_Family):
    def __init__(self, env, gamma=0.995, learning_rate=3e-4, buffer_size=100000,target_action_noise_mul = 1.5, 
                 ...
                 n_step = 1, learning_starts=1000, target_network_update_tau=5e-4, prioritized_replay=False, mixture_type = 'truncated', risk_avoidance = 0.0,
    ...
    def setup_model(self):
        ...
        self.critic = hk.transform(lambda x,a: (Critic(support_n=self.n_support,**self.policy_kwargs)(x,a), Critic(support_n=self.n_support,**self.policy_kwargs)(x,a)))
    ...
    def _loss(self, params, obses, actions, targets, weights, key, step):
        ...
        actor_loss = jnp.mean(-vals*self.policy_weight + 0.01*risk_varience - 0.01*advantage_variance)
