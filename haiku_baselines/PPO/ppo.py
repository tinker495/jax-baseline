...
class PPO(Actor_Critic_Policy_Gradient_Family):
    ...
    def get_logprob_continuous(self, prob, action, key, out_prob=False):
        ...
        std = jnp.exp(log_std)
        if out_prob:
            return prob, - (0.5 * jnp.sum(jnp.square((3.0 * action - mu) / (std + 1e-7)),axis=-1,keepdims=True) + 
                                   jnp.sum(log_std,axis=-1,keepdims=True) + 
                                   0.5 * jnp.log(2 * np.pi))
        else:
            return - (0.5 * jnp.sum(jnp.square((3.0 * action - mu) / (std + 1e-7)),axis=-1,keepdims=True) + 
                             jnp.sum(log_std,axis=-1,keepdims=True) + 
                             0.5 * jnp.log(2 * np.pi))
    ...
    def action_continuous(self,obs,steps):
        ...
        return np.clip(np.random.normal(mu, std) , -3.0, 3.0) / 3.0
