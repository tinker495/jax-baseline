...
class IMPALA_PPO(IMPALA_Family):
    def __init__(self, workers, manager=None, buffer_size=0, gamma=0.995, learning_rate=0.0003, update_freq = 100, batch_size=1024, sample_size=1, epoch_num=3, mini_batch=32, val_coef=0.2, ent_coef=0.01, rho_max=1.2, 
                 clip_value = 100.0, ppo_eps = 0.01, log_interval=1, tensorboard_log=None, _init_setup_model=True, policy_kwargs=None, full_tensorboard_log=False, seed=None, optimizer='adamw'):
        ...
        self.batch_size = int(np.ceil(batch_size / self.mini_batch) * self.mini_batch) + 1
    ...
    def train_step(self, steps):
        ...
        obses, actions, targets, old_prob, rho, adv = self.preprocess(self.params, next(self.key_seq), **data)

        for i in range(self.epoch_num):
            self.params, self.opt_state, critic_loss, actor_loss = \
            self._train_step(self.params, self.opt_state, next(self.key_seq), self.ent_coef, obses, actions, targets, old_prob, rho, adv)
                    
        if steps % self.log_interval == 0:
            log_dict = {"loss/critic_loss": float(critic_loss), "loss/actor_loss": float(actor_loss),
    ...
    def preprocess(self, params, key, obses, actions, mu_log_prob, rewards, nxtobses, dones, terminals):
        ...
        vps = [get_vtrace(rw, p, c, d, t, v, nv, self.gamma) for rw, p, c, d, t, v, nv in zip(rewards, rho, c_t, dones, terminals, value, next_value)]
        adv = [r[:-1] + self.gamma * (1. - d)[:-1] * vp[1:] - v[:-1] for r, d, vp, v in zip(rewards, dones, vps, value)]
        obses = [[o[:-1] for o in ob] for ob in obses]; actions = [a[:-1] for a in actions]; vps = [vp[:-1] for vp in vps]; old_prob = [pi[:-1] for pi in pi_prob]; rho = [r[:-1] for r in rho]
        obses = [jnp.vstack(list(zo)) for zo in zip(*obses)]; actions = jnp.vstack(actions); vps = jnp.vstack(vps); old_prob = jnp.vstack(old_prob); rho = jnp.vstack(rho); adv = jnp.vstack(adv)
        return obses, actions, vps, old_prob, rho, adv
    
    def _train_step(self, params, opt_state, key, ent_coef, obses, actions, targets, old_prob, rho, adv):
        (total_loss, (critic_loss, actor_loss)), grad = jax.value_and_grad(self._loss,has_aux = True)(params, obses, actions, targets, old_prob, rho, adv, ent_coef, key)
        updates, opt_state = self.optimizer.update(grad, opt_state, params=params)
        params = optax.apply_updates(params, updates)
        return params, opt_state, critic_loss, actor_loss


    def _loss_discrete(self, params, obses, actions, targets, old_prob, rho, adv, ent_coef, key):
        ...
        adv = jax.lax.stop_gradient(rho * adv)
    ...
    def _loss_continuous(self, params, obses, actions, targets, old_prob, rho, adv, ent_coef, key):
        feature = self.preproc.apply(params, key, obses)
        vals = self.critic.apply(params, key, feature)
        critic_loss = jnp.mean(jnp.square(jnp.squeeze(targets - vals)))
