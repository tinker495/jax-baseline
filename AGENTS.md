...
이 프로젝트는 JAX JIT을 통해 학습 파이프라인의 반복 연산을 고속화한다. 학습 hot path에는 eager 수치 연산을 남기지 않으며, CPU↔GPU 통신과 동기화를 핵심 병목으로 취급하여 최소화한다. 이 원칙은 알고리즘, 모델, 환경, 데이터 저장 backend와 무관하게 적용한다.

- **범위**: env step·update마다 반복되는 데이터 준비·변환, 추론·샘플링, 상태·통계 갱신, 파라미터 업데이트, 지표 계산·축약을 모두 포함한다. 학습에 필요하지 않은 진단 계산은 필요한 주기에만 수행한다.
- **컴파일 경계**: 각 논리적 계산 단계의 입력 전처리부터 출력 후처리까지 하나의 JIT 함수로 묶어 반복당 compiled 호출 한 번으로 끝낸다. Python 루프에서 장치 배열을 개별 연산하거나 작은 compiled 함수를 연속 호출하는 것으로 대체하지 않는다. dispatch·kernel launch·동기화의 반복 비용까지 최적화 대상이다.
- **데이터와 상태**: JIT 인자는 device array 또는 컴파일 시 고정되는 static 값으로 전달한다. 반복마다 변하는 난수 상태·누적 통계·카운터 등은 device carry로 유지하고 JIT 내부에서 갱신한다. 계산 도중 host scalar나 NumPy로 되돌리지 않으며, 데이터 저장 backend를 eager 계산의 예외로 삼지 않는다.
- **전송 경계**: 초기화·최초 컴파일 같은 1회성 작업과 환경 입출력, 주기적 로깅, 체크포인트·파일 I/O를 내부 계산과 구분한다. 필수 입력은 경계에서 pytree로 묶어 한 번의 `jax.device_put`으로 전달하고, 외부에서 필요한 결과만 묶어 내려받는다. 로깅용 출력은 `log_interval` 경계에서 한 번의 `jax.device_get`으로 모은다. 내부 계산 단계 사이의 불필요한 왕복 전송·스칼라 추출·강제 동기화는 제거한다.
- **전송의 정량 측정**: 컴파일·워밍업을 제외한 고정된 반복 구간에서 CPU→GPU(H2D)와 GPU→CPU(D2H)의 실제 전송 총 횟수와 반복당 횟수를 방향별로 측정한다. 전송량과 전송·동기화 소요 시간도 함께 기록한다. env step 수와 update 수를 구분하고, 실행 조건·측정 구간·전송 발생 위치와 원인을 명시한다. 명시적·암묵적 전송을 모두 포함하며, API 호출 횟수를 실제 메모리 전송 횟수로 간주하지 않는다.
- **최소화 기준**: 데이터와 중간 결과는 소비하는 장치에 유지하고, 필수 전송은 경계에서 묶는다. 동일한 실행 조건에서 변경 전후의 방향별 전송 횟수·전송량·동기화 비용과 전체 처리량을 비교한다. 남겨야 하는 전송은 위치·필요성·횟수를 기록한다. 전송을 줄이기 위해 학습 의미를 바꾸거나, 계측 자체가 반복적인 장치 동기화를 추가해서는 안 된다.
- **검증**: `with jax.transfer_guard("disallow"):`로 hot path의 암묵 전송을 확인한다. 명시적 전송과 device array 사이의 eager 연산은 이 검사만으로 검증되지 않으므로 profiler trace에서 실제 전송·동기화·dispatch·kernel 실행을 확인하고 처리량 측정을 함께 사용한다. guard 통과나 API 호출 수 감소만으로 최적화 완료를 판단하지 않는다.
