...
def resolve_maker(runner: FamilyRunner | DistributedFamilyRunner, spec: AlgoSpec, args: Namespace):
    module = f"{runner.maker_pkg.format(lib=args.model_lib)}.{spec.builder}_builder"
    try:
        mod = import_module(module)
    except ModuleNotFoundError as exc:
        # Convert to a clean "unsupported combo" only when the target builder
        # module itself is missing (an unsupported algo/lib). If a
...
def default_policy_kwargs(args: Namespace) -> dict[str, object]:
    return {} if args.model is None else {"model": load_model_config(args.model)}


def actor_critic_policy_kwargs(args: Namespace) -> dict[str, object]:
    options: dict[str, object] = {}
    for name, path in (("actor_model", args.actor_model), ("critic_model", args.critic_model)):
        if path is not None:
            options[name] = load_model_config(path)
    return options
