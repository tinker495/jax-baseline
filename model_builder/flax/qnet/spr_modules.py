"""Shared SPR/BBF builder internals.

SPR and BBF use byte-identical ``Projection`` / ``Transition`` / ``Prediction`` /
``Model`` modules and an identical ``Merged`` wiring; they differ only in the
PreProcess embedding-mode default and channel ``multiple``. Both builders are thin
