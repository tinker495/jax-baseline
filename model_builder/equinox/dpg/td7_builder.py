...
class Actor(eqx.Module):
    ...
    def __init__(
        ...
        *,
        ...
        keys = jax.random.split(key_rest, hidden_n + 1)
        layers = []
        in_dim = node + zs_dim
        for i in range(hidden_n):
            layers.append(Dense(in_dim, node, key=keys[i]))
...
def model_builder_maker(observation_space, action_size, policy_kwargs):
    ...
    def model_builder(key=None, print_model=False):
        ...
        key_pre, key_enc, key_actor, key_critic = jax.random.split(rng, 4)
        ...
        key_enc_core, key_action_enc = jax.random.split(key_enc)
        encoder_module = EncoderWrapper(
            preproc_module,
            Encoder(feature_dim, node, encoder_hidden, key=key_enc_core),
            ActionEncoder(node, action_size[0], node, encoder_hidden, key=key_action_enc),
        )
        ...
        actor_model = Actor(feature_dim, zs_sample.shape[-1], action_size, node, hidden_n, key=key_actor)
        critic_model = Critic(
            feature_dim,
            zs_sample.shape[-1],
            zsa_sample.shape[-1],
