...
class SimbaDacActor(eqx.Module):
    input_dense: Dense
...
class SimbaOptimisticActor(eqx.Module):
    input_dense: Dense
    blocks: tuple[ResidualBlock, ...]
    norm: LayerNorm
    head: Dense
    ...
    def __init__(
        ...
        *,
        ...
        self.blocks = tuple(ResidualBlock(node, key=keys[i + 1]) for i in range(hidden_n))
        self.norm = LayerNorm(node)
        self.head = Dense(node, action_size[0], key=keys[-1], kernel_init=clip_factorized_uniform(3.0))
...
class SimbaDacCritic(eqx.Module):
    input_dense: Dense
    blocks: tuple[ResidualBlock, ...]
    norm: LayerNorm
    head: Dense
...
class DoubleCritic(eqx.Module):
    critic1: SimbaDacCritic
    critic2: SimbaDacCritic
...
def model_builder_maker(observation_space, action_size, policy_kwargs):
    ...
    def model_builder(key=None, print_model=False):
        ...
        actor_core = SimbaDacActor(feature_dim, action_size, node, hidden_n, key=key_actor)
        ...
        optimistic_actor = SimbaOptimisticActor(feature_dim, action_size, node, hidden_n, key=key_opt_actor)
        ...
        critic_model = DoubleCritic(
            SimbaDacCritic(feature_dim, action_size[0], node, hidden_n, key=key_c1),
            SimbaDacCritic(feature_dim, action_size[0], node, hidden_n, key=key_c2),
        )
