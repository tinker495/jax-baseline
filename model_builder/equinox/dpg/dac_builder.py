...
class ActorCore(eqx.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.mu_head = Dense(hidden_dim, action_size[0], key=mu_key)
        def bias_init(k, shape, dtype=jnp.float32):
            return jnp.full(shape, 10.0, dtype=dtype)
        self.log_std_head = Dense(hidden_dim, action_size[0], key=log_std_key, bias_init=bias_init)
...
class OptimisticActor(eqx.Module):
    backbone: Sequential
    mu_head: Dense
    ...
    def __init__(
        ...
        *,
        ...
        self.backbone, hidden_dim = sequential_dense(feature_dim, node, hidden_n, key=trunk_key)
        self.mu_head = Dense(hidden_dim, action_size[0], key=head_key)
        self.std_multiplier = jnp.log(0.75)
    ...
    def __call__(
        ...
        *,
        ...
        hidden = self.backbone(feature, key=key)
        mu_additional = self.mu_head(hidden, key=key)
        return mu + mu_additional, log_std + self.std_multiplier
...
def model_builder_maker(observation_space, action_size, policy_kwargs):
    ...
    def model_builder(key=None, print_model=False):
        ...
        key_actor, key_opt_actor, key_critic = jax.random.split(rng, 3)
        ...
        optimistic_actor = OptimisticActor(
        ...
        optimistic_fn = get_apply_fn_equinox_module(optimistic_static, optimistic_actor.__call__)
