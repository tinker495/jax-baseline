...
class SimbaV2DacActor(eqx.Module):
...
class SimbaV2OptimisticActor(eqx.Module):
    embedding: SimbaV2Embedding
    blocks: tuple[SimbaV2Block, ...]
    head: SimbaV2Head
    ...
    def __init__(
        ...
        *,
        ...
        self.embedding = SimbaV2Embedding(feature_dim, hidden_dim=node, key=keys[0])
        self.blocks = tuple(SimbaV2Block(node, key=keys[i + 1]) for i in range(hidden_n))
        self.head = SimbaV2Head(node, action_dim, key=keys[-1], use_bias=True)
    ...
    def __call__(
        ...
        *,
        ...
        for block in self.blocks:
            x = block(x)
        mu_additional = self.head(x)
        return mu + mu_additional, log_std + STD_MULTIPLIER


class SimbaV2DacCritic(eqx.Module):
    embedding: SimbaV2Embedding
    blocks: tuple[SimbaV2Block, ...]
    head: SimbaV2Head
...
def model_builder_maker(observation_space, action_size, policy_kwargs):
    ...
    def model_builder(key=None, print_model=False):
        ...
        key_pre, key_actor, key_opt_actor, key_c1, key_c2 = jax.random.split(rng, 5)
        ...
        actor_core = SimbaV2DacActor(feature_dim, action_size[0], node, hidden_n, key=key_actor)
        ...
        optimistic_actor = SimbaV2OptimisticActor(feature_dim, action_size[0], node, hidden_n, key=key_opt_actor)
        optimistic_params_tree, optimistic_static = eqx.partition(optimistic_actor, eqx.is_array)
        optimistic_params = _package_params(optimistic_params_tree)
        optimistic_fn = get_apply_fn_equinox_module(optimistic_static)

        critic_model = eqx.ModuleTuple(
            (
                SimbaV2DacCritic(feature_dim, action_size[0], node, hidden_n, key=key_c1),
                SimbaV2DacCritic(feature_dim, action_size[0], node, hidden_n, key=key_c2),
            )
        )
        critic_params_tree, critic_static = eqx.partition(critic_model, eqx.is_array)
        critic_params = _package_params(critic_params_tree)

        def critic_fn(params, key, feature, action):
            model = eqx.combine(params, critic_static)
            key1, key2 = (None, None) if key is None else jax.random.split(key)
            q1 = model[0](feature, action, key=key1)
            q2 = model[1](feature, action, key=key2)
            return q1, q2
        ...
        if key is not None:
            if print_model:
                ...
                print_param("optimistic_actor", optimistic_params_tree)
        ...
        return preproc_fn, actor_fn, optimistic_fn, critic_fn
