...
class SimbaTQCActor(eqx.Module):
    input_dense: Dense
...
class SimbaTQCCritic(eqx.Module):
    input_dense: Dense
    blocks: tuple[ResidualBlock, ...]
    norm: LayerNorm
    head: Dense
...
class DoubleCritic(eqx.Module):
    critic1: SimbaTQCCritic
    critic2: SimbaTQCCritic
...
def model_builder_maker(observation_space, action_size, support_n, policy_kwargs):
    ...
    def model_builder(key=None, print_model=False):
        ...
        actor_core = SimbaTQCActor(feature_dim, action_size, node, hidden_n, key=key_actor)
        ...
        critic1 = SimbaTQCCritic(feature_dim, action_size[0], node, hidden_n, support_n, key=key_c1)
        critic2 = SimbaTQCCritic(feature_dim, action_size[0], node, hidden_n, support_n, key=key_c2)
