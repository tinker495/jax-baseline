...
class SimbaV2SacActor(eqx.Module):
...
class SimbaV2SacCritic(eqx.Module):
    embedding: SimbaV2Embedding
    blocks: tuple[SimbaV2Block, ...]
    head: SimbaV2Head
...
class DoubleCritic(eqx.Module):
    critic1: SimbaV2SacCritic
    critic2: SimbaV2SacCritic
...
def model_builder_maker(observation_space, action_size, policy_kwargs):
    ...
    def model_builder(key=None, print_model=False):
        ...
        actor_core = SimbaV2SacActor(feature_dim, action_size[0], node, hidden_n, key=key_actor)
        ...
        critic_model = DoubleCritic(
            SimbaV2SacCritic(feature_dim, action_size[0], node, hidden_n, key=key_c1),
            SimbaV2SacCritic(feature_dim, action_size[0], node, hidden_n, key=key_c2),
        )
