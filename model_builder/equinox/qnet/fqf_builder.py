...
class FractionProposal(eqx.Module):
    hidden: tuple
    out: Dense

    def __init__(self, feature_dim: int, support_size: int, *, node: int, hidden_n: int, key: jax.random.KeyArray):
        ...
        self.out = Dense(in_dim, support_size, key=keys[-1])

    def __call__(self, feature: jnp.ndarray) -> tuple[jnp.ndarray, jnp.ndarray, jnp.ndarray]:
        x = feature
        ...
        tau_0 = jnp.zeros((feature.shape[0], 1), dtype=feature.dtype)
        tau_cumsum = jnp.cumsum(probs, axis=-1)
        tau = jnp.concatenate([tau_0, tau_cumsum], axis=-1)
        tau_hat = 0.5 * (tau[:, :-1] + tau[:, 1:])
        entropy = -jnp.sum(log_probs * probs, axis=-1, keepdims=True)
        return tau, tau_hat, entropy


class FQF(eqx.Module):
    feature_proj: Dense
    quantile_proj: Dense
    backbone: tuple
    dueling: bool
        ...
        *,
        ...
        if dueling:
            ...
            self.advantage_head = _init_layer(
                ctor, node, action_dim, keys[head_index + 1]
            )
        else:
...
def model_builder_maker(
    ...
    def model_builder(key=None, print_model=False):
        ...
        key_pre, key_q, key_frac = jax.random.split(rng, 3)
        preproc = PreProcess(
            observation_space,
        ...
        fraction = FractionProposal(feature_dim, n_support, node=node, hidden_n=1, key=key_frac)
        combined = Combined(preproc, qnet)
        combined_params, combined_static = eqx.partition(combined, eqx.is_array)
        fraction_params, fraction_static = eqx.partition(fraction, eqx.is_array)
        preproc_fn = get_apply_fn_equinox_module(combined_static, combined.preprocess)
        q_fn = get_apply_fn_equinox_module(combined_static, combined.q)
        fraction_fn = get_apply_fn_equinox_module(fraction_static)
        if key is not None:
            if print_model:
                print("------------------build-equinox-model--------------------")
                print_param("network", combined_params)
                print_param("fraction", fraction_params)
                print("---------------------------------------------------------")
            return preproc_fn, q_fn, fraction_fn, combined_params, fraction_params
        return preproc_fn, q_fn, fraction_fn
