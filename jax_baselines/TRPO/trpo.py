...
from jax_baselines.TRPO.network import Actor, Critic
...
class TRPO(Actor_Critic_Policy_Gradient_Family):
    def __init__(
        self,
        env,
        gamma=0.995,
    ...
    def train_step(self, steps):
        ...
        obses, actions, targets, value, act_prob, adv = self._preprocess(
            self.params, next(self.key_seq), **data
        )
        ...
        for epoch in range(self.epoch_num):
            ...
            for i in range(len(idxes) // self.minibatch_size):
                ...
                self.params, self.opt_state, c_loss, a_loss = self._optimize_step(
                    self.params,
                    self.opt_state,
                    next(self.key_seq),
                    self.ent_coef,
                    [o[batchidx] for o in obses],
                    actions[batchidx],
                    targets[batchidx],
                    value[batchidx],
                    act_prob[batchidx],
    ...
    def _preprocess(self, params, key, obses, actions, rewards, nxtobses, dones, terminals):
        ...
        act_prob = [
            self.get_logprob(self.actor.apply(params, key, f), a, key)
    ...
    def _loss_discrete(
        self, params, obses, actions, targets, old_value, old_prob, adv, ent_coef, key
    ):
