...
from jax_baselines.TQC_IQN.network import Actor, Quantile_Embeding, Critic
...
class TQC_IQN(Deteministic_Policy_Gradient_Family):
    ...
    def _train_ent_coef(self, log_coef, log_prob):
        l = lambda log_ent_coef, log_prob: -jnp.mean(
            log_ent_coef * (log_prob + self.target_entropy)
        )
    ...
    def _loss(self, params, obses, actions, targets, weights, key, step, ent_coef):
        ...
        huber0 = QuantileHuberLosses(
            jnp.expand_dims(qnets[0], axis=1), logit_valid_tile, huber_tau, self.delta
        )
        ...
        for q in qnets[1:]:
            critic_loss += jnp.mean(
                weights
                * QuantileHuberLosses(
                    jnp.expand_dims(q, axis=1), logit_valid_tile, huber_tau, self.delta
                )
            )
