...
def discount_with_terminal(rewards, dones, terminals, next_values, gamma):
    def f(ret, info):
        ...
        ret = reward + gamma * (ret * (1.0 - term) + nextval * (1.0 - done) * term)
        return ret, ret
    ...
    _, discounted = jax.lax.scan(
        ...
        (rewards, dones, terminals, next_values),
        reverse=True,
    )
    return discounted
...
def get_gaes(rewards, dones, terminals, values, next_values, gamma, lamda):
    ...
    def f(last_gae_lam, info):
        delta, don, term = info
        last_gae_lam = delta + gamma * lamda * (1.0 - don) * (1.0 - term) * last_gae_lam
...
def get_vtrace(rewards, rhos, c_ts, dones, terminals, values, next_values, gamma):
    deltas = rhos * (rewards + gamma * (1.0 - dones) * next_values - values)

    def f(last_v, info):
        delta, c_t, don, term = info
        last_v = delta + gamma * c_t * (1.0 - don) * (1.0 - term) * last_v
        return last_v, last_v

    _, A = jax.lax.scan(
        f,
        jnp.zeros((1,), dtype=jnp.float32),
        (deltas, c_ts, dones, terminals),
        reverse=True,
    )
    v = A + values
    return v
