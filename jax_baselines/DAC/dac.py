...
class DAC(Deteministic_Policy_Gradient_Family):
    ...
    def _select_action_state(self, eval, steps):
        return {
            "pessimistic_policy": self.pessimistic_policy_params,
            "optimistic_policy": self.optimistic_policy_params,
        }

    def _policy_action_from_state(self, state, obs, eval, steps):
        if eval:
            return np.asarray(
                self._get_actions(state["pessimistic_policy"], obs, next(self.key_seq))
            )
        return np.asarray(
            self._get_actions_o(
                state["pessimistic_policy"],
                state["optimistic_policy"],
                obs,
                next(self.key_seq),
            )
        )
