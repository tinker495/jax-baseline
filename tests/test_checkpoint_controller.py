...
def make_controller(**overrides):
    ...
    def pulse(steps, acc):
        calls["pulse"].append((steps, acc))

    def on_episode_end(*args, **kwargs):
        return controller.on_episode_end(
            *args,
            log_metric=lambda k, v, s: calls["logs"].append((k, v, s)),
            **kwargs,
        )

    return controller, calls, pulse, on_episode_end


def test_disabled_short_circuits():
    controller, calls, pulse, on_episode_end = make_controller(use_checkpointing=False)
    assert on_episode_end(10, 5.0, 3, pulse) is True
...
def test_warmup_snapshots_and_returns_without_arming():
    ...
    controller, calls, pulse, on_episode_end = make_controller(steps_before_checkpointing=1000)
    result = on_episode_end(10, 7.0, 4, pulse)
...
def test_non_monitored_worker_feeds_pulse_volume_without_arming():
    ...
    assert on_episode_end(10, 5.0, 3, pulse, advance_criterion=False) is True
    assert calls["snapshot"] == 0
    assert calls["pulse"] == []
    assert calls["logs"] == []

    on_episode_end(20, 7.0, 4, pulse, advance_criterion=True)
    assert calls["snapshot"] == 1
    assert calls["pulse"] == [(20, 7)]


def test_enable_transition():
    controller, _, pulse, on_episode_end = make_controller(steps_before_checkpointing=100)
    on_episode_end(50, 1.0, 1, pulse)
    assert controller.enabled is False
    on_episode_end(150, 1.0, 1, pulse)  # steps now past threshold
    assert controller.enabled is True


def test_refresh_then_below_baseline():
    # Enables immediately (steps_before=0); one episode per window (max_eps=1).
    controller, calls, pulse, on_episode_end = make_controller(
    ...
    assert on_episode_end(10, 10.0, 5, pulse) is True
    assert calls["snapshot"] == 1
    ...
    assert on_episode_end(20, 2.0, 6, pulse) is False
...
def test_window_accumulates_until_max_eps():
    ...
    assert controller.last_update_step is None
    assert on_episode_end(20, 5.0, 3, pulse) is True
    assert calls["snapshot"] == 1  # window full -> refresh
    assert controller.last_update_step == 20


def test_state_round_trip():
    src, _, pulse, on_episode_end = make_controller(max_eps_before_checkpointing=2)
    on_episode_end(10, 5.0, 3, pulse)
    on_episode_end(20, 9.0, 4, pulse)  # drives enable + a refresh
    ...
    dst, _, _, _ = make_controller(max_eps_before_checkpointing=2)
