...
env = UnityEnvironment(file_name="../env/Walker.x86_64",worker_id=0,no_graphics=False,side_channels=[engine_configuration_channel,channel])
env_name = "3DBall"
env_type = "unity"
...
if group_spec.action_spec.continuous_size == 0:
    ...
    conv_action = lambda a: ActionTuple(continuous=np.clip(a, -3.0, 3.0) / 3.0) #np.clip(a, -3.0, 3.0) / 3.0) 
